...
import { mergeTaskDraftAfterPartialSave } from "@/lib/task-draft";
import {
...
export default function TaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function save(
    ...
    try {
      ...
      const serverDetail = body as TaskDetail;
      setDetail((current) => {
        if (!current || options.replaceDraft) return serverDetail;
        return mergeTaskDraftAfterPartialSave(current, serverDetail, patch);
      });
    } catch (saveError) {
  ...
  async function upload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!latest) return;
      await save({ photoIds: [...latest.photoIds, media.id] });
    } catch (uploadError) {
  ...
  async function removePhoto(photoId: string) {
    const latest = detailRef.current;
    if (!latest) return;
    await save({ photoIds: latest.photoIds.filter((value) => value !== photoId) });
  }
    ...
    <main className="min-h-screen bg-background px-4 pb-12 pt-8 sm:px-6">
      <div className="mx-auto max-w-2xl">
        ...
        <Card className="mt-5 space-y-4">
          ...
          <Button fullWidth icon={<Save size={16} />} disabled={saving || uploading || !detail.task.trim()} onClick={() => save(
            {
              task: detail.task,
        ...
        <Card className="mt-4">
          <div className="flex flex-wrap items-center gap-3"><div className="min-w-[12rem] flex-1"><h2 className="font-bold text-text-primary">Photos & video</h2><p className="text-[12px] text-text-secondary">Add up to five photos or short videos for this task.</p></div><button onClick={() => inputRef.current?.click()} disabled={uploading || detail.photos.length >= 5} className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-primary px-3.5 text-[12px] font-semibold text-white disabled:opacity-50">{uploading ? <Loader2 size={14} className="animate-spin" /> : <Camera size={14} />}Add media</button></div>
          <input ref={inputRef} type="file" accept="image/*,video/mp4,video/quicktime,video/webm,.mp4,.mov,.webm" className="hidden" onChange={upload} />
          {detail.photos.length === 0 ? <p className="mt-4 rounded-xl bg-surface-secondary px-4 py-5 text-center text-[12px] text-text-tertiary">No task photos or videos yet.</p> : <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{detail.photos.map((photo) => <div key={photo.id} className="relative overflow-hidden rounded-xl border border-border bg-surface-secondary">
            {isVideoUrl(photo.url)
              ...
              : <img src={photo.url} alt={photo.label ?? "Task photo"} className="aspect-square w-full object-cover" />}<button type="button" onClick={() => removePhoto(photo.id)} className="absolute right-2 top-2 flex h-11 w-11 items-center justify-center rounded-full bg-black/65 text-white" aria-label="Remove media"><X size={14} /></button></div>)}</div>}
        </Card>

        <button
          disabled={saving || uploading}
          onClick={() => save({ status: detail.status === "completed" ? "pending" : "completed" })}
          className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-success/30 bg-success-light text-[13px] font-bold text-success disabled:opacity-50"
        >
          <CheckCircle2 size={16} />
          {detail.status === "completed" ? "Reopen task" : "Mark task complete"}
        </button>
      </div>
    </main>
  );
